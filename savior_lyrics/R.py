...
def name_cleaner(name):
    step1 = artist.lower().replace("&", "and")
    step2 = step1.replace(" ", "-")
    step3 = re.sub(r'[^a-z0-9-]', '', step2)
    corrected_name = re.sub(r'-+', '-', step3).strip('-')
...
def scrape_lyrics(artist, albums):
    ...
    global loses
    data_file = load_file()
    artist_name = name_cleaner(artist).capitalize()

    url = 'https://genius.com/' + artist_name + '-'
    ...
    for album, tracks in albums.items():
        ...
        print('==============', album)
        for track in tracks:
            track_name = name_cleaner(track)
            track_url = url + track_name + '-lyrics'
            ...
            # except requests.exceptions.Timeout:
                    ...
                    print(f"\u001b[41mFailed to retrieve the webpage. {track_url}")
                    print('@', track_url)
                    print(e)
                    loses += 1
                    continue
...
# target_url = "https://genius.com/Arctic-monkeys-cornerstone-lyrics" 
